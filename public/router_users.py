...
@classes_router.post("/", response_model=Union[Main_Classes, New_Respons], tags=[Tags.classes], status_code=status.HTTP_201_CREATED)
def create_class(item: Annotated[Main_Classes, Body(embed=True, description="Новый класс")],
                 DB: Session = Depends(get_session)):
    try:
        new_class = Classes(class_name=item.class_name)
        DB.add(new_class)
        DB.commit()
        DB.refresh(new_class)
        return new_class
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Произошла ошибка при добавлении класса: {str(e)}")
